...
from cambrian.open_clip.transformer import QuickGELU
from cambrian.open_clip.timm_model import TimmModel
...
class CLIP(nn.Cell):
    ...
    def __init__(
        ...
        self.logit_scale = Parameter(Tensor(np.ones([1]) * init_logit_scale, ms.float32), name='logit_scale')
        if init_logit_bias is not None:
            self.logit_bias = Parameter(Tensor(np.ones([1]) * init_logit_bias, ms.float32), name='logit_bias')
        else:
