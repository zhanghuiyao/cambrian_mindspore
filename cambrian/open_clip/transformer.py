...
class Attention(nn.Cell):
    def __init__(
        ...
        if self.scaled_cosine:
            self.logit_scale = Parameter(Tensor(np.log(10 * np.ones((num_heads, 1, 1))), dtype=ms.float32), name='logit_scale')
        else:
            self.logit_scale = None
        self.attn_drop = nn.Dropout(p=attn_drop)
        if self.scale_heads:
            self.head_scale = Parameter(Tensor(np.ones((num_heads, 1, 1)), ms.float32), name='head_scale')
        else:
...
class VisionTransformer(nn.Cell):
    def __init__(
        ...
        scale = width**-0.5
        self.class_embedding = Parameter(scale * np.random.randn(width).astype(np.float32), name='class_embedding')
        self.positional_embedding = Parameter(
            scale * np.random.randn(self.grid_size[0] * self.grid_size[1] + 1, width).astype(np.float32), name='positional_embedding'
        )
        ...
        self.proj = Parameter(Tensor(scale * np.random.randn(width, output_dim), ms.float32), name='proj')
...
class TextTransformer(nn.Cell):
    def __init__(
        ...
        self.text_projection = Parameter(Tensor(np.zeros((width, output_dim)), ms.float32), name='text_projection')

        if embed_cls:
            self.cls_emb = Parameter(Tensor(np.zeros(width), ms.float32), name='cls_emb')
            self.num_pos += 1
        else:
            self.cls_emb = None

        self.token_embedding = nn.Embedding(vocab_size, width)
        self.positional_embedding = Parameter(Tensor(np.zeros((self.num_pos, width)), ms.float32), name='positional_embedding')
        self.transformer = Transformer(
            width=width,
            layers=layers,
            heads=heads,
        ...
        self.attn_mask = Parameter(_attn_mask_tensor, requires_grad=False, name='attn_mask')
