...
class Attention(nn.Cell):
    ...
    def construct(self, x: Tensor) -> Tensor:
        ...
        x = x.swapdims(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class VisionTransformer(nn.Cell):
    ...
    def __init__(
        ...
        self.cls_token = Parameter(Tensor(np.zeros((1, 1, embed_dim)), ms.float32), name='cls_token') if class_token else None
        self.reg_token = Parameter(Tensor(np.zeros((1, reg_tokens, embed_dim)), ms.float32), name='reg_token') if reg_tokens else None
        embed_len = num_patches if no_embed_class else num_patches + self.num_prefix_tokens
        self.pos_embed = Parameter(Tensor(np.random.randn(1, embed_len, embed_dim) * .02, ms.float32), name='pos_embed')
    ...
    def _pos_embed(self, x: Tensor) -> Tensor:
        ...
        if self.cls_token is not None:
            to_cat.append(self.cls_token.broadcast_to((x.shape[0], -1, -1)))
        if self.reg_token is not None:
            to_cat.append(self.reg_token.broadcast_to((x.shape[0], -1, -1)))
    ...
    def _intermediate_layers(
        ...
        take_indices = list(range(num_blocks - n, num_blocks)) if isinstance(n, int) else n
...
def resize_pos_embed(
    ...
    *DEPRECATED* This function is being deprecated in favour of resample_abs_pos_embed
    ...
    if not len(gs_new):  # backwards compatibility
        gs_new = [int(ntok_new ** 0.5)] * 2
    assert len(gs_new) >= 2
    _logger.info(f'Resized position embedding: {posemb.shape} ({[gs_old, gs_old]}) to {posemb_new.shape} ({gs_new}).')
    posemb_grid = posemb_grid.reshape(1, gs_old, gs_old, -1).permute(0, 3, 1, 2)
    # FIXME: `antialias` not support in `mindspore.ops.interpolate`
    posemb_grid = ops.interpolate(posemb_grid, size=gs_new, mode=interpolation, align_corners=False)
    posemb_grid = posemb_grid.permute(0, 2, 3, 1).reshape(1, gs_new[0] * gs_new[1], -1)
    posemb = ops.cat([posemb_prefix, posemb_grid], axis=1)
    return posemb
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = '') -> None:
    ...
    if hasattr(model.patch_embed, 'backbone'):
        ...
        stem.conv.weight.set_data(adapt_input_conv(stem.conv.weight.shape[1], _n2p(w[f'{prefix}conv_root/kernel'])))
        stem.norm.weight.set_data(_n2p(w[f'{prefix}gn_root/scale']))
        stem.norm.bias.set_data(_n2p(w[f'{prefix}gn_root/bias']))
        if not stem_only:
            for i, stage in enumerate(backbone.stages):
                for j, block in enumerate(stage.blocks):
                    bp = f'{prefix}block{i + 1}/unit{j + 1}/'
                    for r in range(3):
                        getattr(block, f'conv{r + 1}').weight.set_data(_n2p(w[f'{bp}conv{r + 1}/kernel']))
                        getattr(block, f'norm{r + 1}').weight.set_data(_n2p(w[f'{bp}gn{r + 1}/scale']))
                        getattr(block, f'norm{r + 1}').bias.set_data(_n2p(w[f'{bp}gn{r + 1}/bias']))
                    if block.downsample is not None:
                        block.downsample.conv.weight.set_data(_n2p(w[f'{bp}conv_proj/kernel']))
                        block.downsample.norm.weight.set_data(_n2p(w[f'{bp}gn_proj/scale']))
                        block.downsample.norm.bias.set_data(_n2p(w[f'{bp}gn_proj/bias']))
    ...
    model.patch_embed.proj.weight.set_data(embed_conv_w)
    model.patch_embed.proj.bias.set_data(_n2p(w[f'{prefix}embedding/bias']))
    if model.cls_token is not None:
        model.cls_token.set_data(_n2p(w[f'{prefix}cls'], t=False))
    if big_vision:
    ...
    model.pos_embed.set_data(pos_embed_w)
    model.norm.weight.set_data(_n2p(w[f'{prefix}Transformer/encoder_norm/scale']))
    model.norm.bias.set_data(_n2p(w[f'{prefix}Transformer/encoder_norm/bias']))
    if (isinstance(model.head, nn.Dense) and
            f'{prefix}head/bias' in w and
            model.head.bias.shape[0] == w[f'{prefix}head/bias'].shape[-1]):
        model.head.weight.set_data(_n2p(w[f'{prefix}head/kernel']))
        model.head.bias.set_data(_n2p(w[f'{prefix}head/bias']))
    # NOTE representation layer has been removed, not used in latest 21k/1k pretrained weights
    # if isinstance(getattr(model.pre_logits, 'fc', None), nn.Linear) and f'{prefix}pre_logits/bias' in w:
    #     model.pre_logits.fc.weight.set_data(_n2p(w[f'{prefix}pre_logits/kernel']))
    #     model.pre_logits.fc.bias.set_data(_n2p(w[f'{prefix}pre_logits/bias']))
    if model.attn_pool is not None:
        ...
        for r in range(2):
            getattr(model.attn_pool.mlp, f'fc{r + 1}').weight.set_data(_n2p(w[f'{block_prefix}MlpBlock_0/Dense_{r}/kernel']))
            getattr(model.attn_pool.mlp, f'fc{r + 1}').bias.set_data(_n2p(w[f'{block_prefix}MlpBlock_0/Dense_{r}/bias']))

    mha_sub, b_sub, ln1_sub = (0, 0, 1) if big_vision else (1, 3, 2)
    for i, block in enumerate(model.blocks.cell_list()):
        ...
        for r in range(2):
            getattr(block.mlp, f'fc{r + 1}').weight.set_data(_n2p(w[f'{block_prefix}MlpBlock_{b_sub}/Dense_{r}/kernel']))
            getattr(block.mlp, f'fc{r + 1}').bias.set_data(_n2p(w[f'{block_prefix}MlpBlock_{b_sub}/Dense_{r}/bias']))
...
def _convert_openai_clip(
    ...
    for k, v in state_dict.items():
        ...
        if k == 'proj':
            k = 'head.weight'
            v = v.swapdims(0, 1)
            out_dict['head.bias'] = ops.zeros(v.shape[0])
