...
from cambrian.timm.layers.adaptive_avgmax_pool import SelectAdaptivePool2d
from cambrian.timm.layers.create_norm import get_norm_layer
...
def _create_fc(num_features, num_classes, use_conv=False):
    if num_classes <= 0:
        ...
        fc = nn.Conv2d(num_features, num_classes, 1, pad_mode='pad', has_bias=True)
    else:
        fc = nn.Dense(num_features, num_classes, pad_mode='pad', has_bias=True)
    return fc
...
class NormMlpClassifierHead(nn.Cell):
    ...
    def __init__(
        ...
        linear_layer = partial(nn.Conv2d, kernel_size=1, pad_mode='pad', has_bias=True) if self.use_conv else nn.Dense
    ...
    def reset(self, num_classes, pool_type=None):
        ...
        if self.hidden_size:
            if ((isinstance(self.pre_logits.fc, nn.Conv2d) and not self.use_conv) or
                ...
                new_fc.weight.set_data(self.pre_logits.fc.weight.reshape(new_fc.weight.shape))
                new_fc.bias.set_data(self.pre_logits.fc.bias)
